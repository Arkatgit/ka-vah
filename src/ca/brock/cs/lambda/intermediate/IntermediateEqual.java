...
public class IntermediateEqual extends IntermediateTerm {
    private IntermediateTerm left;
    private IntermediateTerm right;

    private static final int precedence = 10; // Precedence for equality

    public IntermediateEqual(IntermediateTerm left, IntermediateTerm right) {
        this.left = left;
        this.right = right;
    }
...
//    /**
//     * For comparison operations, methodT() recursively calls methodT() on its operands.
//     * @return A new IntermediateEqual with transformed operands.
//     */
//    @Override
//    public IntermediateTerm methodT() {
//        return new IntermediateEqual(left.methodT(), right.methodT());
//    }
    /**
     * For comparison operations, methodT() recursively calls methodT() on its operands.
    ...
    public IntermediateTerm methodT(boolean optimize) {
        return new IntermediateEqual(left.methodT(optimize), right.methodT(optimize));
    }

    /**
     * Converts this IntermediateEqual to a CombinatorApplication representing the operation.
     * T[L = R] = (( = T[L]) T[R])
     * @return The equivalent Combinator term.
     */
    @Override
    public Combinator toCombinatorTerm() {
        Combinator eqOp = new CombinatorConstant("=");
        Combinator translatedLeft = left.toCombinatorTerm();
        Combinator translatedRight = right.toCombinatorTerm();
        return new CombinatorApplication(new CombinatorApplication(eqOp, translatedLeft), translatedRight);
    }
}
