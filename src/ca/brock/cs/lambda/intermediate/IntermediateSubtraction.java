...
public class IntermediateSubtraction extends IntermediateTerm {
    private IntermediateTerm left;
    private IntermediateTerm right;

    private static final int precedence = 20;

    public IntermediateSubtraction(IntermediateTerm left, IntermediateTerm right) {
        this.left = left;
        this.right = right;
    }
...
//     * @return A new IntermediateSubtraction with transformed operands.
//     */
//    @Override
//    public IntermediateTerm methodT() {
//        return new IntermediateSubtraction(left.methodT(), right.methodT());
//    }
    /**
     * For arithmetic operations, methodT() recursively calls methodT() on its operands.
    ...
    public IntermediateTerm methodT(boolean optimize) {
        return new IntermediateSubtraction(left.methodT(optimize), right.methodT(optimize));
    }

    /**
     * Converts this IntermediateSubtraction to a CombinatorApplication representing the operation.
     * T[L - R] = ((- T[L]) T[R])
     * @return The equivalent Combinator term.
     */
    @Override
    public Combinator toCombinatorTerm() {
        Combinator subOp = new CombinatorConstant("-");
        Combinator translatedLeft = left.toCombinatorTerm();
        Combinator translatedRight = right.toCombinatorTerm();
        return new CombinatorApplication(new CombinatorApplication(subOp, translatedLeft), translatedRight);
    }
}
