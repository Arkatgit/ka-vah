...
//                String name = entry.getKey();
//
//                // 1. Get the initial intermediate term
//                ca.brock.cs.lambda.intermediate.IntermediateTerm term = funcDef.getTerm().toIntermediateTerm();
//                Combinator combinator = null;
//                int maxIterations = 100; // Safeguard against infinite loops
//
//                // 2. Loop methodT() to catch any abstractions dynamically generated by Match desugaring
//                while (maxIterations-- > 0) {
//                    term = term.methodT(true);  // Translate abstractions to combinators
//
//                    try {
//                        // 3. Attempt to finalize the combinator tree
//                        combinator = term.toCombinatorTerm();
//                        break; // Success! All abstractions have been removed.
//                    } catch (IllegalStateException e) {
//                        // If we encounter the abstraction error, it means a Match node desugared
//                        // into new abstractions that need to be processed. Let the loop run again.
//                        if (maxIterations == 0 || !e.getMessage().contains("Cannot convert IntermediateAbstraction")) {
//                            throw e; // Rethrow if we run out of tries or if it's a different error
//                        }
//                    }
//                }
//
    ...
    public static Map<String, Combinator> translateToRawCombinators(
        ...
        for (Map.Entry<String, DefinedValue> entry : symbolMap.entrySet()) {
            if (entry.getValue() instanceof FunctionDefinition) {
                ...
                Term scottEncodedTerm = ScottEncoding.desugar(funcDef.getTerm());
                Combinator combinator = scottEncodedTerm
                    .toIntermediateTerm()
                    .methodT(true)
                    .toCombinatorTerm();
...
// 4. Desugar ADTs/match to Scott-encoded core terms before combinator lowering
        Map<String, DefinedValue> scottEncodedSymbolMap = ScottEncoding.desugarProgram(optimizedSymbolMap);
        System.out.println("\n--- Scott Encoded Terms ---");
        scottEncodedSymbolMap.forEach((k, v) -> System.out.println(k + " = " + v));

// 5. Translate Scott-encoded core terms to raw combinators
        Map<String, Combinator> rawCombinators = translateToRawCombinators(scottEncodedSymbolMap);
        System.out.println("\n--- Raw Combinators ---");
        rawCombinators.forEach((k, v) -> System.out.println(k + " = " + v));
    ...
    public static void main(String[] args) {
        ...
        String[] testPrograms = {
            "data Box a = Emptybox | Fullbox a;\n" +
                "\n" +
                "unbox : Box Int -> Int;\n" +
                "unbox = λb. match b with\n" +
                "    Emptybox -> 0\n" +
                "  | Fullbox val -> val\n" +
                "end;\n" +
                "\n" +
                "main = unbox (Fullbox 42);",
            ...
            "data list a = emptylist | cons a (list a);\n" +
                "sum : list Int -> Int;\n" +
                "sum = rec sumit. λxs. match xs with\n" +
                "    emptylist -> 0\n" +
                "    | cons x xs_tail -> x + (sumit xs_tail)\n" +
                "end;\n" +
           ...
           "data list a = emptylist | cons a (list a);\n" +
               "\n" +
               "filterLE : (a -> a -> Bool) -> a -> list a -> list a;\n" +
               "filterLE = rec fle. λleq. λpivot. λxs.\n" +
               "    match xs with\n" +
               "        emptylist -> emptylist\n" +
               "        | cons x xstail ->\n" +
               "            if leq x pivot\n" +
               "            then cons x (fle leq pivot xstail)\n" +
               "            else fle leq pivot xstail\n" +
               "    end;\n" +
               "\n" +
               "filterGT : (a -> a -> Bool) -> a -> list a -> list a;\n" +
               "filterGT = rec fgt. λleq. λpivot. λxs.\n" +
               "    match xs with\n" +
               "        emptylist -> emptylist\n" +
               "        | cons x xstail ->\n" +
               "            if leq x pivot\n" +
               "            then fgt leq pivot xstail\n" +
               "            else cons x (fgt leq pivot xstail)\n" +
               "    end;\n" +
               "\n" +
               ...
               "    match xs with\n" +
               "        emptylist -> ys\n" +
               "        | cons x xstail -> cons x (app xstail ys)\n" +
               "    end;\n" +
               "\n" +
               "quicksort : (a -> a -> Bool) -> list a -> list a;\n" +
               "quicksort = rec qsort. λleq. λxs.\n" +
               "    match xs with\n" +
               "        emptylist -> emptylist\n" +
               "        | cons pivot rest ->\n" +
               "            append (qsort leq (filterLE leq pivot rest)) \n" +
               "                   (cons pivot (qsort leq (filterGT leq pivot rest)))\n" +
               "    end;\n" +
               "\n" +
               "leqInt : Int -> Int -> Bool;\n" +
               "leqInt = λx. λy. x <= y;\n" +
               "\n" +
               "myList : list Int;\n" +
               "myList = cons 3 (cons 1 (cons 4 (cons 1 (cons 5 (cons 9 (cons 2 (cons 6 (cons 5 (cons 3 (cons 5 emptylist))))))))));\n" +
               "\n" +
               "main : list Int;\n" +
               "main = quicksort leqInt myList;"
